...
spsa = SPSA(maxiter=150,trust_region=True,learning_rate=0.07,perturbation=0.1)
...
E_4q_p1_zneexp_kolkata,angles_4q_p1_zneexp_kolkata = vqe_ap_hea_zne(graph=graph,
                                            ansatz_str=ansatz_str,
                                            g_values=g_values,
                                            layers=layers,
                                            options=options_mitigated_nairobi,
                                            optimizer=optimizer,
                                            init_optimizer=initial_optimizer,
...
plt.plot(g_values[::-1],E_4q_p1_zneexp_kolkata,'o',label='noisy')
plt.xlabel('g',fontsize=fontsize)
plt.ylabel('E',fontsize=fontsize)
plt.legend()
plt.savefig(path+f'graphE_{n_qubits}q_{ansatz_str}{layers}_zne_{zne}_{extrap}_{server}.pdf',format='pdf')
